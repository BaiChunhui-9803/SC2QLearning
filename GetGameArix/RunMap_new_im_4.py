...
class SmartAgent(Agent):
    ...
    def get_state_by_area(self, obs):
        my_units = self.get_my_units_by_type(obs, _MY_UNIT_TYPE_ARG)
        enemy_units = self.get_enemy_units_by_type(obs, _ENEMY_UNIT_TYPE_ARG)
        unit_my_list = sorted([(item['tag'], item['x'], item['y']) for item in my_units], key=lambda x: x[0])
        unit_enemy_list = sorted([(item['tag'], item['x'], item['y']) for item in enemy_units], key=lambda x: x[0])
        map_my = [0] * 100
        map_enemy = [0] * 100
        for point in unit_my_list:
            tag, x, y = point
            region_x = int(x / _MAP_RESOLUTION * 10)
            region_y = int(y / _MAP_RESOLUTION * 10)
            # print(x, region_x, y, region_y)
            map_my[region_x * 10 + region_y] += 1
            # print(map_my)
        for point in unit_enemy_list:
            tag, x, y = point
            region_x = int(x / _MAP_RESOLUTION * 10)
            region_y = int(y / _MAP_RESOLUTION * 10)
            # print(region_x, region_y)
            map_enemy[region_x * 10 + region_y] += 1
            # print(map_enemy)
        str_my = ''.join(map(str, map_my))
        str_enemy = ''.join(map(str, map_enemy))
        str_all = str_my + str_enemy
        # print(str_all)
        return str_all
    ...
    def step(self, obs):
        ...
        state = str(self.get_state_by_area(obs))
