...
2023-04-04
"""
def update_unit_map(my_units):
    print(UNIT_MAP)
    tag_list = [str(unit['tag']) for unit in my_units]
    cooldown_list = [unit['weapon_cooldown'] for unit in my_units]
    health_list = [unit['health'] for unit in my_units]
    for unit_tag in tag_list:
        for key, value in UNIT_MAP.items():
            if value[0] == unit_tag:
                print('找到了')


def draw_im(window_map: np.ndarray, my_units):
    norm = mcolors.TwoSlopeNorm(vmin=_MIN_INFLUENCE, vmax=_MAX_INFLUENCE, vcenter=0.0)
    p1 = sns.heatmap(window_map, cmap="RdBu", norm=norm,
    ...
    # plt.clf()
    # print(UNIT_MAP)
    update_unit_map(my_units)
    plt.subplot(2, 1, 2)
    plt.bar(tag_list, health_list, label='Health', color='blue')
    plt.bar(tag_list, cooldown_list, label='Cooldown', color='red')
    plt.pause(0.1)
    plt.clf()
    # plt.pause(0.1)
    # print(tag_list)
...
def draw_data(my_units):
    units = ['Unit 1', 'Unit 2', 'Unit 3', 'Unit 4', 'Unit 5', 'Unit 6', 'Unit 7', 'Unit 8']
    tag_list = [unit['tag'] for unit in my_units]
    cooldown_list = [unit['weapon_cooldown'] for unit in my_units]
    health_list = [unit['health'] for unit in my_units]
    plt.clf()
    plt.subplot(2, 1, 2)
    plt.bar(units, health_list, label='Health', color='blue')
    plt.bar(units, cooldown_list, label='Cooldown', color='red')
    plt.pause(0.1)
    # print(tag_list)
    # print(cooldown_list)
    return
...
class SmartAgent(Agent):
    ...
    def step(self, obs):
        ...
        if obs.first():
            ...
            # 添加单位至单元map
            for i in range(len(unit_list_my)):
                UNIT_MAP[str(i)] = [unit_list_my[i]['tag'], unit_list_my[i]['weapon_cooldown'], unit_list_my[i]['health']]
        ...
        # 开启数据分析模式
        draw_im(self.get_window_im(obs), my_units)
        # draw_data(my_units)
        # state = len(state_vec)
        # !!!!!!!
